...
public class CustomNativeMappingServiceTest extends AbstractMetadataTest {
    ...
    public void testCustomToNative() {
        ...
        map.get(String.class, "refsystem-as-list", 0)
            .setValue("foo");
        map.get(String.class, "refsystem-as-list", 1)
            .setValue("bar");
        map.subMap("contact", 0)
            .get(String.class, "name")
            .setValue("DOV");
        map.subMap("contact", 1)
            .get(String.class, "name")
            .setValue("Vlaanderen");
        map.get(String.class, "identifier-single")
            .setValue("1234");
        ...
        assertEquals("https://www.dov.vlaanderen.be/geonetwork/?uuid=1234", layer.getResource().getMetadataLinks().get(0).getContent());
        
        assertEquals("text/html", layer.getResource().getMetadataLinks().get(0).getType());
        
        assertEquals("ISO191156:2003", layer.getResource().getMetadataLinks().get(0).getMetadataType());
        
        assertEquals("https://www.dov.vlaanderen.be/geonetwork/srv/nl/csw?Service=CSW&Request=GetRecordById&Version=2.0.2&outputSchema=http://www.isotc211.org/2005/gmd&elementSetName=full&id=1234", 
        ...
        assertEquals("ISO191156:2003", layer.getResource().getMetadataLinks().get(1).getMetadataType());
    ...
    public void testNativeToCustom() {
        ...
        layer.getResource().getKeywords().add(
                new Keyword("KEY_foo"));
        ...
        layer.getResource().getMetadataLinks().get(0)
                .setContent("https://www.dov.vlaanderen.be/geonetwork/?uuid=1234");
        layer.getResource().getMetadataLinks().get(0)
                .setType("text/html");
        layer.getResource().getMetadataLinks().get(0)
        .setMetadataType("ISO191156:2003");
        layer.getResource().getMetadataLinks().add(new MetadataLinkInfoImpl());
        layer.getResource().getMetadataLinks().get(1)
                .setContent("https://www.dov.vlaanderen.be/geonetwork/srv/nl/csw?Service=CSW&Request=GetRecordById&Version=2.0.2&outputSchema=http://www.isotc211.org/2005/gmd&elementSetName=full&id=1234");
        layer.getResource().getMetadataLinks().get(1)
                .setType("text/xml");
        layer.getResource().getMetadataLinks().get(1)
                .setMetadataType("ISO191156:2003");
        layer.getIdentifiers().add(new LayerIdentifier());
        layer.getIdentifiers().get(0)
                .setIdentifier("abcde");
        layer.getIdentifiers().get(0)
                .setAuthority("DOV-be");
        layer.getIdentifiers().add(new LayerIdentifier());
        layer.getIdentifiers().get(1)
                .setIdentifier("fghi");
        layer.getIdentifiers().get(1)
                .setAuthority("EOV-ce");
        ...
        assertEquals("foo", map.get(String.class, "refsystem-as-list", 0)
            .getValue());
        assertEquals("bar", map.get(String.class, "refsystem-as-list", 1)
            .getValue());
        assertEquals(2, map.size("contact"));  
        assertEquals("DOV", map.subMap("contact", 0)
            .get(String.class, "name")
            .getValue());
        assertEquals("Vlaanderen", map.subMap("contact", 1)
            .get(String.class, "name")
            .getValue());
        assertEquals("1234", map.get(String.class, "identifier-single")
            .getValue());
        assertEquals(2, map.size("referencesystem-object"));  
        assertEquals("abcde", map.subMap("referencesystem-object", 0)
            .get(String.class, "code")
            .getValue());
        assertEquals("DOV-be", map.subMap("referencesystem-object", 0)
            .get(String.class, "code-space")
            .getValue());
        assertEquals("fghi", map.subMap("referencesystem-object", 1)
            .get(String.class, "code")
            .getValue());
        assertEquals("EOV-ce", map.subMap("referencesystem-object", 1)
            .get(String.class, "code-space")
            .getValue());
    }

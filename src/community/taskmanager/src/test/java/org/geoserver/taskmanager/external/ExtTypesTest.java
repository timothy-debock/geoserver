...
public class ExtTypesTest extends AbstractTaskManagerTest {
    ...
    public void testTableName() {
        ...
        final String tableName = "vw_grondwaterlichamen";
        ...
        try (Connection conn = source.getDataSource().getConnection()) {
            try (ResultSet res = conn.getMetaData().getTables(null, source.getSchema(), tableName, null)) {
        ...
        assertTrue(extTypes.tableName.parse(tableName, Collections.singletonList("myjndidb")) instanceof DbSource);
        assertNull(extTypes.tableName.parse("doesntexist", Collections.singletonList("myjndidb")));
    }
    ...
    public void testFileServices() {
        ...
        assertEquals("s3-test-source", domain.get(1));
        assertEquals("s3-test-target", domain.get(2));
        assertEquals("temp-directory", domain.get(3));
